"""
Gemma 4-E4B-it — LoRA Fine-Tuning for Arabic Diacritization
============================================================
...
Model         : google/gemma-4-E4B-it   (LoRA — bf16 base, adapters trained)
...
1. HF_TOKEN below is intentionally empty. Paste a token, or better, export it:
   ...
   the same account, or the download fails with 401 after the job has queued.
...
OUTPUT_DIR = f"./gemma-4-e4b-lora-diacritization{_suffix}"
...
# just the ones that are a real torch.nn.Linear — which matters on Gemma 4:
#
#   model.language_model.*  -> 258 plain nn.Linear        (adapted)
#   model.vision_tower.*    -> 120 Gemma4ClippableLinear  (skipped)
#   audio encoder           ->     Gemma4ClippableLinear  (skipped)
#
# All three reuse these same seven names. Gemma4ClippableLinear wraps nn.Linear
# but inherits from nn.Module, and PEFT type-checks before filtering, so passing
# these names straight to LoraConfig dies with
#   ValueError: Target module Gemma4ClippableLinear(...) is not supported
# (huggingface/peft#3129). The isinstance filter below is what avoids that while
# keeping all seven projections from Confg_Info.md.
LORA_TARGET_MODULES = [
...
# run_gemma_lora.sbatch: the smallest round limit that covers the slow end.
NUM_TRAIN_EPOCHS = 1
...
# ============================================================================
# FAIRNESS CONTRACT — must be IDENTICAL in the Qwen3.5-4B script
# ============================================================================
# These two models are compared on the same benchmark, so every
# optimisation-relevant quantity must match: effective batch, LR/schedule,
# epochs, LoRA rank/alpha/dropout/targets, MAX_SEQ_LENGTH, seed, splits.
# Only PER_DEVICE_TRAIN_BATCH_SIZE may differ (gemma-4-E4B is 8.6B with a
# 262k vocabulary vs Qwen's 4.2B/248k), with accumulation derived so the
# product is unchanged. Under DDP the world size multiplies in too:
#     effective batch = PER_DEVICE * ACCUM * NUM_GPUS
EFFECTIVE_BATCH = 96          # 3 GPUs; 64 is not divisible by 3

PER_DEVICE_TRAIN_BATCH_SIZE = 4
GRADIENT_ACCUMULATION_STEPS = None   # derived below — do not set by hand

# gemma-4-E4B is 8.6B (~17GB bf16) with a larger vocabulary than Qwen, so
# activations are kept checkpointed here. Qwen also runs with this ON, so the
# comparison is not confounded by one model recomputing and the other not.
...
if EFFECTIVE_BATCH % _per_step_without_accum != 0:
    raise SystemExit(
        f"FATAL: EFFECTIVE_BATCH={EFFECTIVE_BATCH} not divisible by "
        f"PER_DEVICE({PER_DEVICE_TRAIN_BATCH_SIZE}) x WORLD_SIZE({WORLD_SIZE}). "
        f"The two models would train on different effective batches and stop "
        f"being comparable. Adjust PER_DEVICE.")
...
def hf_login():
    ...
    if not token:
        print("[WARN] No HF token set. google/gemma-4-E4B-it is gated and the download "
...
def load_model_and_tokenizer():
    ...
    load_kwargs = dict(
        ...
        # DDP: each rank needs a FULL copy on ITS OWN gpu. device_map="auto"
        # would shard one copy across all visible GPUs (model parallel), which
        # torchrun/DDP does not expect.
...
def build_tokenizer_fn(tokenizer):
    def tokenize(ex):
        prompt_text = tokenizer.apply_chat_template(
            build_prompt_messages(ex[INPUT_COLUMN]),
            tokenize=False, add_generation_prompt=True,
        )
...
def plot_training_curves(trainer):
    ...
    plt.ylabel("Loss")
    plt.title("Gemma 4-E4B LoRA — Train vs. Test Loss")
    plt.legend()
    plt.grid(alpha=0.3)
        ...
        with torch.no_grad():
            for start in range(0, len(order), INFER_BATCH_SIZE):
                ...
                try:
                    prompts = [
                        tokenizer.apply_chat_template(
                            build_prompt_messages(pairs[i][0]),
                            tokenize=False, add_generation_prompt=True)
                        for i in idxs
                    ]
...
def plot_performance_comparison(metrics_df):
    ...
    plt.title("Gemma 4-E4B LoRA — Train vs. Test vs. Benchmark")
    plt.legend()
    plt.grid(alpha=0.3)
