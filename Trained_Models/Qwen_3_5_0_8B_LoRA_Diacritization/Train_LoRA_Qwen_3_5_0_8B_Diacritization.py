"""Qwen3.5-0.8B LoRA fine-tuning for Arabic diacritization.

The default training run is one complete epoch over every usable row in the pinned
Misraj/Sadeed_Tashkeela training split. It has no step cap and no cost-based early stop.
Set MAX_STEPS or MAX_TRAIN_COST_USD explicitly only when an intentionally partial run is wanted.

The model is Qwen/Qwen3.5-0.8B with a frozen bf16 backbone and fp32 LoRA adapters. Qwen3.5-0.8B
is a hybrid model, so the adapter targets cover both full-attention and Gated-DeltaNet layers;
select_lora_targets() asserts all 150 expected target modules before training begins.

Training, evaluation, scoring, checkpoint resume, and dataset preparation live in this file.
The Modal wrapper gives a new full-epoch run its own Volume namespace, preserving the earlier
checkpoint-1800 adapter while preventing it from being resumed by the new run.
"""
...
# As close to container-billed wall clock as this process can observe: modal_app.py's _run()
# launches this script as a fresh subprocess per container, so this is set within a few seconds
# of the GPU actually starting to be billed. CostBudgetCallback measures elapsed time from here,
# not from trainer.train(), so it also counts model load + tokenization, not just optimizer
# steps -- both cost real money on the same container.
...
OUTPUT_DIR = os.environ.get("OUTPUT_ROOT", "./qwen3.5-0.8b-lora-diacritization")
...
# ---- training ----
# -1 lets num_train_epochs control the run. The default is one full pass over the complete
# training split. Positive values intentionally override the epoch count.
NUM_TRAIN_EPOCHS = float(os.environ.get("NUM_TRAIN_EPOCHS", "1"))
MAX_STEPS = int(os.environ.get("MAX_STEPS", "-1"))

# 8 x 8 = effective batch 64, IDENTICAL to the 4B LoRA run and to the full-FT script this was
# forked from. Not tuned here on purpose: keeping the optimization geometry fixed is what makes
# the benchmark numbers comparable.
#
# The plan this package implements argued per-device 16 (accum 4) would fit, on the reasoning
# that LoRA frees the ~12GB of fp32 weights/grads/AdamW state a full fine-tune spends on all
# 751M params. MEASURED, IT DOES NOT: smoke_test at batch 16 hit
#     CUDA out of memory. Tried to allocate 15.16 GiB ... 64.13 GiB already in use (of 79.18 GiB)
# on the longest length-grouped megabatch — the ceiling is the LOGIT tensor (batch x seq x
# 248,320, upcast to fp32 for cross-entropy, PLUS its bf16 copy PLUS its gradient), and that
# scales with batch regardless of how many parameters LoRA freezes. This is exactly the fallback
# the plan's own gate 9 anticipated ("if it exceeds ~70GB, fall back to 8/8"), so that's the
# default here — not 16. Accumulation is 8, matching, so the EFFECTIVE batch is still 64.
...
SMOKE_RUN_EVAL = os.environ.get("SMOKE_RUN_EVAL", "0") == "1"

# Opt-in throughput experiments. Liger's fused linear cross-entropy is especially relevant here:
# Qwen3.5 has a 248,320-token vocabulary, so the unfused [batch, sequence, vocabulary] logits
# tensor dominated both memory and the previous smoke test. Keep these environment-controlled so
# the original recipe remains available as an A/B baseline.
USE_LIGER_KERNEL = os.environ.get("USE_LIGER_KERNEL", "0") == "1"
TORCH_COMPILE = os.environ.get("TORCH_COMPILE", "0") == "1"
TORCH_COMPILE_MODE = os.environ.get("TORCH_COMPILE_MODE", "reduce-overhead")
# Liger/Triton specializes its fused loss for tensor shapes. Exact dynamic padding produced a
# late 29-second JIT pause in the first speed smoke. Bucketing to multiples of 64 bounds the run
# to 16 sequence shapes while retaining nearly all of length grouping's padding savings.
PAD_TO_MULTIPLE_OF = int(os.environ.get(
    "PAD_TO_MULTIPLE_OF", "64" if USE_LIGER_KERNEL else "0"))
if PAD_TO_MULTIPLE_OF < 0:
    raise ValueError("PAD_TO_MULTIPLE_OF must be >= 0")

# The 10x LoRA rule (Thinking Machines' "LoRA Without Regret", confirmed independently by "LoRA
# Learns Less and Forgets Less"), NOT the full-FT script's 2e-5. LoRA adapters start at zero and
# need a larger LR to move in a comparable number of steps; every weight in a full fine-tune is
# already pretrained and 2e-4 would wreck them. Holding 2e-5 here would measure LoRA at the
# wrong learning rate, not LoRA — this is the one deliberate break from "hold everything fixed"
# and it is called out in the README.
LEARNING_RATE = float(os.environ.get("LEARNING_RATE", "2e-4"))
LR_SCHEDULER_TYPE = "cosine"
# ENV-SETTABLE for the fairness-contract run. LoRA_Fine_Tuning_Config_Comparison.md
# pins warmup at 5% of total steps ("Warmup as a ratio, not a fixed step count"); this
# script shipped 0.03. The default stays 0.03 so the r32 full-epoch run remains
# reproducible; the contract run sets WARMUP_RATIO=0.05 explicitly.
...
# Raised from the full-FT script's 2 -> 5: an adapter here is ~110MB, not ~9GB, so keeping more
# validation rungs costs little storage.
SAVE_TOTAL_LIMIT = int(os.environ.get("SAVE_TOTAL_LIMIT", "5"))

# ENV-SETTABLE. The contract requires load_best_model_at_end=False -- the LAST checkpoint
# is the deliverable. Its "Why the final checkpoint, not the 'best' one" section is not a
# style note: with it on, the 30%-data Gemma run shipped checkpoint-200 of 3,259 and
# scored WORSE than the 10% run, because eval_loss over 500 rows is noisy and dipped
# early. Default stays True so the r32 run reproduces; the contract run sets 0.
LOAD_BEST_MODEL_AT_END = os.environ.get("LOAD_BEST_MODEL_AT_END", "1") == "1"

# ---- LoRA ----
# r=64, alpha=128 (alpha = 2r, "crucial for high ranks" per LoRA Learns Less) is the plan doc's
# recommendation, sized against the ~50M supervised tokens the 2.58 full-FT checkpoint actually
# saw (it was step 4,070 of 16,293) rather than a full epoch: at 2 bits/param, r=64 gives 82M
# bits of capacity against that ~50M-token target. No rank probe was run to establish an optimum,
# so r=64 remains the configuration of the partial run for a clean continuation of the experiment.
LORA_R = int(os.environ.get("LORA_R", "64"))
LORA_ALPHA = int(os.environ.get("LORA_ALPHA", str(2 * LORA_R)))
# 4B script used 0.05. With ~1M examples seen once, dropout here would mainly add gradient noise.
LORA_DROPOUT = float(os.environ.get("LORA_DROPOUT", "0.0"))
# Exact suffixes this hybrid model needs — see select_lora_targets() for why the conventional
# 7-name list misses 18 of 24 layers, and for the 150-module count this is asserted against.
LORA_TARGET_SUFFIXES = (
    "q_proj", "k_proj", "v_proj", "o_proj",                 # full attention  — 6 layers x 4
    "in_proj_qkv", "in_proj_z", "out_proj",                 # Gated-DeltaNet  — 18 layers x 3
    "gate_proj", "up_proj", "down_proj",                    # MLP             — 24 layers x 3
)
EXPECTED_LORA_TARGET_COUNT = 6 * 4 + 18 * 3 + 24 * 3  # = 150

# ---- optional cost budget ----
# A positive value watches the actual wall clock of the current container, including model load
# and tokenization, and stops once the configured amount is reached.
# Disabled by default so a normal run cannot stop before completing the epoch. Set a positive
# value explicitly to make a deliberately budget-capped run.
TRAIN_COST_HOURLY_USD = float(os.environ.get("TRAIN_COST_HOURLY_USD", "4.58"))
MAX_TRAIN_COST_USD = float(os.environ.get("MAX_TRAIN_COST_USD", "0"))
# Small on purpose ("not too large to miss the time"): at ~0.75s/step this is a ~15s check
# interval, so worst-case overshoot past the budget is a couple of cents, not a couple of
# dollars.
COST_CHECK_EVERY_N_STEPS = int(os.environ.get("COST_CHECK_EVERY_N_STEPS", "20"))
...
# ENV-SETTABLE. The contract pins this at 500 ("In-training eval subset | 500 rows of the
# test split | Same eval cost during training"). The 1,000 above was justified by
# eval_loss selecting the final checkpoint -- but under the contract
# load_best_model_at_end is False, so eval_loss selects nothing and is purely diagnostic.
# Default unchanged at 1000 so the r32 run reproduces.
IN_TRAINING_EVAL_SUBSET = int(os.environ.get("IN_TRAINING_EVAL_SUBSET", "1000"))
...
def report_parameters(model):
    ...
    embed = embed_trainable = 0
    for name, module in model.named_modules():
        if module.__class__.__name__ == "Embedding" and hasattr(module, "weight"):
            embed = max(embed, module.weight.numel())
            embed_trainable = embed_trainable or module.weight.requires_grad

    print(f"[INFO] total params      {total:,}")
    print(f"[INFO] trainable params  {trainable:,}  ({100 * trainable / max(total, 1):.2f}%)")
    if embed and embed_trainable:
        # tie_word_embeddings=true in this config, so the input embedding and lm_head are ONE
        # tensor: for a FULL fine-tune this is ~34% of the trainable parameters — the
        # 248,320-row vocabulary table, most of whose rows are non-Arabic and receive
        # essentially no gradient on this corpus. Only fires when embed_tokens is actually
        # trainable; under LoRA it is deliberately excluded from LORA_TARGET_SUFFIXES (see
        # select_lora_targets), so this branch should never print for this package.
        ...
        print(f"[INFO] tied embedding    {embed:,}  (frozen — not a LoRA target, "
              f"deliberately: adapting it would defeat LoRA's point, see select_lora_targets)")
...
def select_lora_targets(model):
    """Full dotted paths of the nn.Linear modules LoRA should wrap on this hybrid model.

    Qwen3.5-0.8B mixes two layer kinds (config.json `layer_types`, full_attention_interval=4):
    6 full-attention layers (`self_attn.{q,k,v,o}_proj`) and 18 Gated-DeltaNet linear-attention
    layers (`linear_attn.{in_proj_qkv,in_proj_z,in_proj_a,in_proj_b,out_proj}`); all 24 layers
    have `mlp.{gate,up,down}_proj`. PEFT's target_modules matches on leaf-name SUFFIX, so the
    conventional 7-name list (q/k/v/o_proj + gate/up/down_proj — what ../Train-Qwen used and what
    the tracked LoRA fairness contract specifies) reaches attention in only 6 of 24 layers:
    `o_proj` does not match `out_proj`, so every Gated-DeltaNet projection — 38% of the
    adaptable weight, measured from the checkpoint's safetensors header — gets nothing. This is
    the single highest-leverage decision in the whole run (LoRA Learns Less: correct module
    targeting beats a 16x rank increase), so LORA_TARGET_SUFFIXES adds in_proj_qkv/in_proj_z/
    out_proj and the module count is ASSERTED, not trusted.

    Deliberately excluded from LORA_TARGET_SUFFIXES:
      in_proj_a, in_proj_b — shape [16, 1024]. out_features=16, so any r>16 over-parameterizes a
                              16-dim output. These are the delta-rule decay gates; error here
                              compounds through the recurrence rather than staying local.
      conv1d                — depthwise Conv1d, groups=6144. Not a sane LoRA target.
      lm_head / embed_tokens — tie_word_embeddings=true; embed_tokens alone is 254M params
                              (33.9% of the model). Adapting it defeats LoRA's point.
      visual.* / mtp.*      — not in the causal-LM forward path (see assert_text_only). Excluded
                              by name here rather than relied on to simply not match, matching
                              the 4B script's select_lora_targets().
    """
    import torch
    ...
    targets = [
        ...
        and name.split(".")[-1] in LORA_TARGET_SUFFIXES
        and not any(part in excluded for part in name.split("."))
    ]
    if not targets:
        raise RuntimeError(
            f"none of {LORA_TARGET_SUFFIXES} exist as nn.Linear in {MODEL_ID} outside the "
            f"{excluded} towers. Print model.named_modules() and fix LORA_TARGET_SUFFIXES."
        )

    attn = [t for t in targets if t.split(".")[-1] in ("q_proj", "k_proj", "v_proj", "o_proj")]
    lin_attn = [t for t in targets
                if t.split(".")[-1] in ("in_proj_qkv", "in_proj_z", "out_proj")]
    mlp = [t for t in targets if t.split(".")[-1] in ("gate_proj", "up_proj", "down_proj")]
    attn_layers = {t.split(".layers.")[1].split(".")[0] for t in attn if ".layers." in t}
    lin_attn_layers = {t.split(".layers.")[1].split(".")[0] for t in lin_attn if ".layers." in t}
    mlp_layers = {t.split(".layers.")[1].split(".")[0] for t in mlp if ".layers." in t}
    print(f"[INFO] LoRA targets: {len(targets)} total — "
          f"{len(attn)} full-attention across {len(attn_layers)} layers, "
          f"{len(lin_attn)} Gated-DeltaNet across {len(lin_attn_layers)} layers, "
          f"{len(mlp)} MLP across {len(mlp_layers)} layers")

    # THE GATE. A count that isn't exactly 150 means PEFT's suffix matching or this model's
    # architecture changed since this list was written — training on a silently different
    # target set is exactly the crippled-adapter bug this fork exists to fix, so stop rather
    # than proceed on a guess.
    if len(targets) != EXPECTED_LORA_TARGET_COUNT:
        raise RuntimeError(
            f"expected exactly {EXPECTED_LORA_TARGET_COUNT} LoRA target modules "
            f"(6x4 full-attn + 18x3 linear-attn + 24x3 mlp), got {len(targets)}. Inspect "
            f"model.named_modules() before training — this is the bug that silently crippled "
            f"the 4B LoRA run (see the plan doc / module docstring)."
        )
    print(f"[OK] LoRA target count == {EXPECTED_LORA_TARGET_COUNT}, as expected.")

    found_leaves = {t.split(".")[-1] for t in targets}
    missing = set(LORA_TARGET_SUFFIXES) - found_leaves
    if missing:
        print(f"[WARN] no nn.Linear found in the text decoder for {sorted(missing)}")
    return targets


def load_model_for_training():
    """Tokenizer + bf16-base/fp32-adapter LoRA model, every precondition asserted before step 1.

    The base stays frozen in bf16 — full fine-tuning's fp32-master-weights requirement applies
    only to TRAINABLE parameters (assert_master_dtype filters on p.requires_grad), and here
    that's ~41M adapter parameters, not 751M.
    """
    import torch
    ...
    # bf16, not fp32: the base is frozen, so it never needs an fp32 master copy. No device_map:
    # on one GPU it only installs accelerate hooks and sets hf_device_map, which makes Trainer
    # think this is model-parallel.
    model = _from_pretrained(
        AutoModelForCausalLM, MODEL_ID, torch.bfloat16,
    ...
    if hasattr(model, "enable_input_require_grads"):
        # Belt-and-braces: gradient checkpointing is off here, but if a future edit turns it on
        # over a frozen base, missing this severs the autograd graph ("element 0 of tensors does
        # not require grad"). get_peft_model() also calls this once a PEFT config is attached on
        # transformers >= ~4.35, so this mainly protects against an older stack.
        model.enable_input_require_grads()

    targets = select_lora_targets(model)
    model = get_peft_model(model, LoraConfig(
        ...
        use_rslora=False,   # rsLoRA scales alpha/sqrt(r), which would change the effective
                            # scale per rank — not a concern with a fixed r=64, kept explicit.
        use_dora=False,     # within 0.43% of vanilla LoRA per 2026 surveys once LR is tuned,
                            # for 5-10% more VRAM. Not worth the variable here.
        init_lora_weights=True,   # not PiSSA (needs its own lower LR), not LoftQ (memory spikes).
    ))
    model.print_trainable_parameters()

    # THE fp32-MASTER-WEIGHTS INVARIANT, LoRA EDITION. PEFT creates adapters in the base model's
    # dtype. On a bf16 base that means bf16 adapters, which means bf16 AdamW moments — the same
    # trap assert_master_dtype guards against for full FT, where 1.0 + 1e-4 == 1.0 and small
    # updates round away. lr=2e-4 makes it less fatal than 2e-5 would, not harmless. Cast the
    # TRAINABLE params up; the frozen base stays bf16 on purpose (report_parameters below prices
    # both).
    for _, p in model.named_parameters():
        if p.requires_grad:
            p.data = p.data.float()
    assert_master_dtype(model)   # reused unchanged: it already filters on p.requires_grad
...
def load_model_for_eval(model_path: str):
    ...
    For an adapter dir, loads the frozen bf16 base + the adapter, then merges: unmerged PEFT
    inference is slower and this also matches what prepare_model_for_generation() does on the
    end-to-end path, so both paths score the same numbers rather than differing in the last
    decimal.
    """
    import torch
    from transformers import AutoModelForCausalLM, AutoTokenizer
    ...
    if base_only:
        ...
        base = _from_pretrained(AutoModelForCausalLM, MODEL_ID, torch.bfloat16,
                                cache_dir=CACHE_DIR, trust_remote_code=TRUST_REMOTE_CODE)
        assert_text_only(base)
        model = PeftModel.from_pretrained(base, src)
        model = model.merge_and_unload()
        print(f"[OK] merged LoRA adapter from {src} into the bf16 base")

    if torch.cuda.is_available():
        model = model.to("cuda")
    model.config.use_cache = True
    model.eval()
    print(f"[OK] {'BASE MODEL (zero-shot baseline)' if base_only else 'LoRA fine-tuned model'}: {src}")
...
def _make_training_trainer(Trainer):
    ...
    class _FullFTTrainer(Trainer):
        ...
        def evaluate(self, *args, **kwargs):
            # The Liger fused-linear-CE patch used during training is not taken by Qwen3.5's
            # evaluation forward path in transformers 5.14.1. Empty cached training blocks
            # before the ordinary loss materializes [batch, seq, 248320] logits.
            import torch
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            print(f"[EVAL] cleared CUDA cache; eval batch={self.args.eval_batch_size}")
        ...
        def report(self, state, prefix=""):
            ...
            if projection_steps:
                ...
                print(f"{prefix}[THROUGHPUT] {projection_label}: {projection_steps:,} steps = "
                      f"{hours:.2f} h wall clock, ~${hours * TRAIN_COST_HOURLY_USD:.2f} at "
                      f"${TRAIN_COST_HOURLY_USD:.2f}/hr (gpu=H100:1, cpu=8, memory=32GiB)")
                print(f"{prefix}[THROUGHPUT] excludes {N_SAVES} checkpoint writes of ~110MB each "
                      f"(adapter, not full weights) and {N_EVALS} in-training evals — those add "
                      f"low single-digit minutes, not the ~30-45 min a full-FT run pays.")
            return s_per_step

    return ThroughputCallback()


def _make_cost_budget_callback():
    """Stop training the moment projected spend on THIS container crosses MAX_TRAIN_COST_USD.

    Why a wall-clock budget and not just a step cap: a step cap trusts the smoke test's measured
    s/step, and a length-grouped sampler's step time varies ~2x across a 50-step megabatch cycle
    (RUN_REPORT.md §5.6 — this project has already been burned by extrapolating from too few
    steps). This callback instead watches the actual clock, so it is right regardless of whether
    the measurement was.

    Checked every COST_CHECK_EVERY_N_STEPS steps, not once at the end — deliberately small so a
    check that fires late costs cents, not dollars. Timed from _SCRIPT_START_TIME (process start),
    not from trainer.train(), so model load and tokenization time — which bill the same GPU
    container — count against the budget too.

    A soft stop, not a hard kill: control.should_training_stop=True lets Trainer finish the
    current step and run its normal end-of-training save, so whatever the best checkpoint is at
    that point lands on disk rather than being lost to a killed process.
    """
    from transformers import TrainerCallback

    class CostBudgetCallback(TrainerCallback):
        def on_train_begin(self, args, state, control, **kwargs):
            if MAX_TRAIN_COST_USD > 0:
                already = (time.time() - _SCRIPT_START_TIME) / 3600 * TRAIN_COST_HOURLY_USD
                print(f"[BUDGET] hard stop at ${MAX_TRAIN_COST_USD:.2f} for this container "
                      f"(${already:.2f} already spent on load/tokenize before step 1), checked "
                      f"every {COST_CHECK_EVERY_N_STEPS} steps at ${TRAIN_COST_HOURLY_USD:.2f}/hr")
            else:
                print("[BUDGET] MAX_TRAIN_COST_USD=0 -- cost budget callback disabled")

        def on_step_end(self, args, state, control, **kwargs):
            if MAX_TRAIN_COST_USD <= 0:
                return
            step = state.global_step
            if step == 0 or step % COST_CHECK_EVERY_N_STEPS != 0:
                return
            elapsed_h = (time.time() - _SCRIPT_START_TIME) / 3600
            cost = elapsed_h * TRAIN_COST_HOURLY_USD
            print(f"[BUDGET] step {step:,}: {elapsed_h * 3600:.0f}s since process start, "
                  f"~${cost:.2f} spent on this container")
            if cost >= MAX_TRAIN_COST_USD:
                print(f"[BUDGET] STOP -- ${cost:.2f} >= ${MAX_TRAIN_COST_USD:.2f} budget. "
                      f"Stopping now; Trainer will save before exiting.")
                control.should_training_stop = True

    return CostBudgetCallback()
...
def schedule_for(total_steps: int, n_evals: int = N_EVALS, n_saves: int = N_SAVES) -> dict:
    ...
    """
    fixed = int(os.environ.get("EVAL_STEPS", "0"))
    if fixed > 0:
        # The contract pins logging/eval/save cadence at 20/200/200. This script instead
        # scaled the interval to the run length (N_EVALS=20), so EVAL_STEPS was read by
        # nobody -- the Ibex sbatch exported it and it was a no-op. Honour it when set,
        # and keep save_steps == eval_steps, which schedule_for.__doc__ requires.
        print(f"[INFO] EVAL_STEPS={fixed} pinned by environment "
              f"(adaptive default would have been {max(1, total_steps // max(1, n_evals))})")
        return {"eval_steps": fixed, "save_steps": fixed}
...
def _pick_optim():
    ...
    Not a micro-optimization even at this run's scale (thousands, not tens of thousands, of
    steps over the ~41M fp32 adapter parameters at r=64): the fused kernel replaces a few
    hundred small elementwise launches per step with one. It is the same maths — fused AdamW is
...
def train_model(model, tokenizer, train_raw, test_raw, smoke_test=False, max_steps=None):
    ...
    if effective_max_steps and effective_max_steps > 0:
        ...
        if frac < 1.0 and not smoke_test:
            print("[WARN] This is a partial-dataset run. Remove the positive MAX_STEPS override "
                  "or pass --max-steps -1 to train for the configured full epoch.")
    else:
    ...
    args_kwargs = dict(
        ...
        load_best_model_at_end=LOAD_BEST_MODEL_AT_END and not smoke_test,
    ...
    if USE_LIGER_KERNEL:
        # Patch only the loss bottleneck during the first experiment. This keeps RMSNorm/SwiGLU
        # unchanged, making any throughput delta attributable to fused linear cross-entropy.
        args_kwargs.update(
        ...
        print("[OPT] Liger fused linear cross-entropy enabled")
    if PAD_TO_MULTIPLE_OF:
        print(f"[OPT] padding length-grouped batches to multiples of {PAD_TO_MULTIPLE_OF} "
              "to bound shape-specialized kernel compilation")
    if TORCH_COMPILE:
        args_kwargs.update(
            torch_compile=True,
        ...
        print(f"[OPT] torch.compile enabled (inductor, mode={TORCH_COMPILE_MODE})")
    if not smoke_test:
        ...
        print(f"[INFO] {total_steps:,} steps -> eval every {sched['eval_steps']:,}, "
              ...
              f"(adapter checkpoint; SAVE_TOTAL_LIMIT={SAVE_TOTAL_LIMIT})")
    ...
    trainer_kwargs = dict(
        ...
        eval_dataset=eval_tok if (not smoke_test or SMOKE_RUN_EVAL) else None,
        ...
        callbacks=[_make_loss_sanity_callback(), throughput, _make_cost_budget_callback()],
    )
    ...
    if smoke_test and SMOKE_RUN_EVAL:
        print("[SMOKE] running the real evaluation forward path as an OOM gate")
        metrics = trainer.evaluate()
        print(f"[SMOKE] evaluation gate passed: eval_loss={metrics.get('eval_loss')}")
    ...
    if not smoke_test:
        ...
        save_adapter(trainer, tokenizer, total_steps, provenance)
...
def save_adapter(trainer, tokenizer, total_steps, provenance=None):
    """Adapter weights + tokenizer + provenance.

    ~110MB, not the ~3GB full fp32 checkpoint would cost — but NOT self-contained: the base
    model must be reloaded alongside it at eval time (see load_model_for_eval /
    prepare_model_for_generation, both of which merge_and_unload() before generating).

    trainer.save_model() on a PeftModel calls PeftModel.save_pretrained(), which writes only the
    adapter (adapter_config.json + adapter_model.safetensors) by default — the frozen base is
    never re-serialized.
    ...
    try:
        ...
        with open(os.path.join(FINAL_MODEL_DIR, "run_config.json"), "w", encoding="utf-8") as f:
            json.dump({
                ...
                "lora_target_module_count": EXPECTED_LORA_TARGET_COUNT,
                ...
                "max_train_cost_usd": MAX_TRAIN_COST_USD,
            }, f, indent=2)
        print(f"[OK] Saved LoRA adapter -> {FINAL_MODEL_DIR}")
    except Exception as e:
        print(f"[WARN] Failed to save the adapter: {e}")
        traceback.print_exc()
...
def plot_training_curves(trainer):
    ...
    plt.ylabel("Loss")
    plt.title(f"{MODEL_ID} LoRA r={LORA_R} — Train vs. Test Loss")
    plt.legend()
    plt.grid(alpha=0.3)
...
def prepare_model_for_generation(model):
    ...
    Three changes, and skipping any of them is expensive or wrong rather than merely untidy:

    1. merge_and_unload(). Unmerged PEFT inference runs the adapter as an extra pair of matmuls
       per targeted Linear on every forward pass — slower, and it is also what makes this path
       match load_model_for_eval()'s disk-reload path score the same numbers.

    2. use_cache=True. load_model_for_training() sets it False, because a KV cache is useless
    ...
    3. bf16. The trainABLE adapter params carry fp32 master weights during training (see
       assert_master_dtype), but after merge_and_unload() the merged weights inherit the frozen
       base's bf16 dtype already — this cast is now mostly a no-op, kept for the case merging
       ever leaves a stray fp32 tensor behind, and to match load_model_for_eval()'s bf16 load.
    ...
    if hasattr(model, "merge_and_unload"):
        model = model.merge_and_unload()
        print("[OK] merged LoRA adapter into base weights for generation")
...
def plot_performance_comparison(metrics_df):
    ...
    plt.title(f"{MODEL_ID} LoRA r={LORA_R} — by split (corrected scorer)")
...
def parse_args():
    p = argparse.ArgumentParser(description="LoRA fine-tune of Qwen3.5-0.8B for Arabic "
                                            "diacritization (one full epoch by default)")
    p.add_argument("--rank", type=int, default=None,
                   help=f"override LORA_R={LORA_R} (also sets LORA_ALPHA=2*rank unless "
                        f"LORA_ALPHA is set in the environment). No probe was run to justify "
                        f"a different value under this run's budget — see the module docstring.")
    ...
    p.add_argument("--label", default="lora",
    ...
    p.add_argument("--max-steps", type=int, default=None,
                   help=f"override MAX_STEPS={MAX_STEPS}; -1 uses NUM_TRAIN_EPOCHS="
                        f"{NUM_TRAIN_EPOCHS} (the default full-dataset run)")
...
def main():
    ...
    if args.rank is not None:
        ...
        LORA_R = args.rank
        if "LORA_ALPHA" not in os.environ:
            LORA_ALPHA = 2 * LORA_R
        print(f"[INFO] --rank override: LORA_R={LORA_R}, LORA_ALPHA={LORA_ALPHA}")

    print(f"[INFO] MODEL_ID={MODEL_ID}")
    print(f"[INFO] OUTPUT_DIR={OUTPUT_DIR}")
    print(f"[INFO] EVAL_DIR={EVAL_DIR}")
    print(f"[INFO] LORA_R={LORA_R} LORA_ALPHA={LORA_ALPHA} LORA_DROPOUT={LORA_DROPOUT}")
    print(f"[INFO] LEARNING_RATE={LEARNING_RATE} PER_DEVICE_TRAIN_BATCH_SIZE="
          f"{PER_DEVICE_TRAIN_BATCH_SIZE} GRADIENT_ACCUMULATION_STEPS="
          f"{GRADIENT_ACCUMULATION_STEPS}")
    if MAX_TRAIN_COST_USD > 0:
        print(f"[INFO] MAX_TRAIN_COST_USD={MAX_TRAIN_COST_USD} "
              f"COST_CHECK_EVERY_N_STEPS={COST_CHECK_EVERY_N_STEPS} "
              f"TRAIN_COST_HOURLY_USD={TRAIN_COST_HOURLY_USD}")
    ...
    run_evaluation(model, tokenizer, train_raw, test_raw, benchmark_raw,
                   splits=splits, label="lora", model_source=FINAL_MODEL_DIR)
