...
"""## Step 6 - DER/WER metrics, CORRECTED scorer (copied from Evaluation_Functions_Corrected.py)
...
it just gets scored as an error for that word.

This block replaces the project's old inline scorer, which had five
confirmed defects: no NFC normalization, diacritics compared as a flat
per-word list with the host letter discarded, a DER denominator that moved
with the prediction instead of being fixed by the reference,
...
- **Metrics are self-contained:** all DER/WER functions live in Step 6 of this script, using the CORRECTED scorer (see Evaluation_Functions_Corrected.py at the repo root) -- no external `Models_Functions.py` or Sadeed repo clone required.
"""
